...
extension UITapGestureRecognizer {
    ...
    func didTapAttributedTextInLabel(label: UILabel, inRange targetRange: NSRange) -> Bool {
        ...
        let textContainerOffset = CGPoint(
            x: (label.bounds.size.width - textBoundingBox.size.width) * ScreenSize.heightAspectRatio - textBoundingBox.origin.x,
            y: (label.bounds.size.height - textBoundingBox.size.height) * ScreenSize.heightAspectRatio - textBoundingBox.origin.y
        )
        ...
        // Check if the tapped index is within the "Read Less" range
        if NSLocationInRange(indexOfCharacter, targetRange) {
            print("Tapped on 'Read Less'")
            return true // "Read Less" was tapped
        }
...
extension UILabel {
    ...
    @objc private func didTapOnMoreText(_ recognizer: UITapGestureRecognizer) {
        ...
        if let tapIndex = tapIndex {
            ...
            if recognizer.didTapAttributedTextInLabel(label: self , inRange: lessTextRange) {
                if isExpanded {
                    isExpanded = false
                    print("Tapped on Read Less")
                    comp?(false)
